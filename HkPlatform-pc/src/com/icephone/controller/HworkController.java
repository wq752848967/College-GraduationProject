...
public class HworkController {
	...
	@ResponseBody
	@RequestMapping(value="/getHworkListByStatusAndDate",method=RequestMethod.POST)
	public Map<String,Object> getHworkListByStatusAndDate(@RequestParam int status,
			@RequestParam int pageNow,@RequestParam int pageSize)
	{
		System.out.println("get work List  by date");
		
		
		List list =  null;
		if(status==Constants.HWORK_STATUS_ALL){
			list = hworkService.getAllByDate();
		}
		else{
			list = hworkService.getAllByStatus(status);
		}
		
		//devide page
		Page page = new Page();
		list  = page.getPageList(list, pageNow, pageSize);
		Map map = ResponseMapUtil.responseSuccess("success", list);
	...
	@ResponseBody
	@RequestMapping(value="/getHworkListUserful",method=RequestMethod.POST)
